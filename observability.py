...
def configure_observability():
    provider=TracerProvider(resource=Resource.create({"service.name":"enterprise-rag-platform","deployment.environment":os.getenv("ENVIRONMENT","local")}))
...
class PrincipalObservabilityMiddleware(BaseHTTPMiddleware):
    async def dispatch(self,request:Request,call_next)->Response:
        ...
        start=time.perf_counter(); status=500; error_type=None
        tracer=trace.get_tracer("principal-http")
        try:
            with tracer.start_as_current_span(f"{request.method} {request.url.path}") as span:
                span.set_attribute("request_id",request_id); span.set_attribute("correlation_id",correlation_id)
            ...
            request.state.latency_ms=elapsed; request.state.status=status; request.state.error_type=error_type
