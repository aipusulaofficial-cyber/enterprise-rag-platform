...
class RetrievalThresholdError(RAGError): pass

def chunk_document(doc: Document, size: int = 120, overlap: int = 20) -> list[Chunk]:
    if size <= overlap or size <= 0: raise ValueError("size must be > overlap >= 0")
...
class Retriever:
    def __init__(self, chunks: list[Chunk], threshold: float=0.1): self.chunks,self.threshold=chunks,threshold
    def retrieve(self, query: str, k: int=5) -> list[Citation]:
        if not query.strip(): raise ValueError("query is required")
        scored=sorted(((lexical_score(query,c.text),c) for c in self.chunks), key=lambda x:(-x[0],x[1].id))
        hits=[Citation(c.id,c.source,s) for s,c in scored if s>=self.threshold][:k]
        if not hits: raise RetrievalThresholdError("no grounded chunks met threshold")
        return hits

def main():
    q=" ".join(sys.argv[1:]) or "architecture"
    docs=[Document("demo","Production RAG systems require retrieval, grounding, citations and observability.","demo://document")]
    r=Retriever(chunk_document(docs[0],20,5))
    for x in r.retrieve(q): print(x)
